 ...
 * @property Post $post
 */
class PostLike extends EMongoDocument
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('likepost, dislikepost', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, likepost, dislike, create_time, user_id, post_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('likepost',$this->like);
	...
	public function isUserLike($user_id , $post_id )
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.
		...
		// $postLike =  self::model()->find( $criteria);

		$array = array(
			'condition' => array( 'user_id' => $user_id  , 'post_id'=>$post_id   ) ,
		);

		$criteria = new EMongoCriteria($array);
		$postLike =  self::model()->findOne( $criteria);

		if($postLike == null ) return $postLike ;
		return  $postLike->likepost;
	}
